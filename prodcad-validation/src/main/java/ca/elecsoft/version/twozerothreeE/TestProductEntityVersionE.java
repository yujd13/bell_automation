...
/**
 * This class is responsible for running the product comparisions from the expected to actual
 */
public class TestProductEntityVersionE {
    List<ProdCadEVersionE> listE;


    /**
     * This function is the one that performs the validation and inserts the compares into a test report
     */
    @Test
    public void testProdCad() {
        // this object is responsible for all database queries
        HibernateUtilVersionE util = HibernateUtilVersionE.getINSTANCE();
        //this is getting all expected products to be tested
        List<ProdCadEVersionE> list = util.displayAllProdCadE();
        //this function add the table headers into the report
        Reporter.log("Product Code| Sheet Product Property | CRM Product Property | Actual | Expected | Pass/Fail");

        //iterating through all products for comparisons
        for (ProdCadEVersionE e : list) {

            String productId = e.getProductid();
            //getting the actual product information for comparisons
            ProdCadAVersionE a = util.getOneProdCadA(productId);
            // making sure that the actual and expected both exist
            if (a != null && e != null) {
                // this is where the col by col comparisons is happening and being reported
                ...
                //this is reporting that the actual porduct does not exist in the system
                Reporter.log(e.getProductid() + "|" + "Product ID" + "|" + "MISSING" + "|" + e.getProductid() + "|" + "false");
            }

            //this is just checking if the function ran succesfully
